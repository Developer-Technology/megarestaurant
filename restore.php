...
function run_split_sql($uploadfile, $host, $usuario,$passwd){
    ...
    $sql="";
    // CONEXION 
    $conexion = new mysqli('localhost','megainve_restaof','Antigua20tenx!','megainve_restaoficial',3306) or die ("No se puede conectar con el servidor MySQL: %s\n". $conexion->connect_error);
    ...
    foreach ($strSQLs as $key => $line){
     ...
     echo "";
                        if (!$conexion->ping() ){ 
							...
							$conexion = new mysqli('localhost','root','','restaurant',3306) or die ("No se puede conectar con el servidor MySQL: %s\n". $conexion->connect_error);
                            $respuesta = $conexion->query($sql);
