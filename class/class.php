...
############################# FUNCION PARA ACCEDER AL SISTEMA DE VENTA ############################
    public function Logueo()
    {
        ...
        $sql = " select * from usuarios LEFT JOIN cajas ON usuarios.codigo = cajas.codigo WHERE usuarios.usuario = ? and usuarios.password = ? and usuarios.status = 'ACTIVO'";
        ...
        if ($num == 0) {
            ...
            if ($p[0]["nivel"] == 'CAJERO' and $p[0]["codcaja"] == '') {

                echo "<div class='alert alert-danger'>";
                echo "<span class='fa fa-info-circle'></span> NO TIENE CAJA DE VENTA ASIGNADA";
                echo "</div>";
                exit;
...
//cerramos la lectura del archivo "abrir archivo" con un "cerrar archivo"
            ...
            echo "<div class='alert alert-success'>";
            echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
            echo "<span class='fa fa-check-square-o'></span> LA CARGA DE PRODUCTOS FUE REALIZADA EXITOSAMENTE";
            echo "</div>";
            exit;
...
############################## FUNCION LISTAR VENTAS EN MOSTRADOR ##################################
    ...
    {
        self::SetNames();
        $sql = "SELECT ventas.idventa, ventas.codventa, ventas.codcliente as cliente, ventas.totalpago, ventas.cocinero, clientes.codcliente, clientes.cedcliente, clientes.nomcliente, salas.nombresala, mesas.nombremesa, GROUP_CONCAT(cantventa, ' | ', producto SEPARATOR '<br>') AS detalles FROM ventas INNER JOIN detalleventas ON detalleventas.codventa = ventas.codventa LEFT JOIN clientes ON ventas.codcliente = clientes.codcliente INNER JOIN mesas ON mesas.codmesa = ventas.codmesa LEFT JOIN salas ON mesas.codsala = salas.codsala WHERE ventas.cocinero = '1' GROUP BY detalleventas.codventa";
        foreach ($this->dbh->query($sql) as $row) {
            $this->p[] = $row;
        }
        return $this->p;
